...
//  			vecUp		= m_qOiginal * Vector3.up;
			...
			vecUp = Camera.mainCamera.transform.rotation * Vector3.up;

		if (m_bCameraLookAt)
			transform.LookAt(Camera.mainCamera.transform, vecUp);
		else
			transform.LookAt(transform.position + Camera.mainCamera.transform.rotation * Vector3.back, vecUp);
		...
		case AXIS_TYPE.AXIS_RIGHT:
			transform.Rotate(transform.up, 270, Space.World);
			break;
		...
		case AXIS_TYPE.AXIS_DOWN:
			transform.Rotate(transform.right, 270, Space.World);
			break;
		}
