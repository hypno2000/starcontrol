...
public class FXMakerLayout : NgLayout
{
	...
	public		const string	m_CurrentVersion			= "v1.3.1";
...
// 	public		static Color	m_ColorButtonActive			= new Color(0.95f, 1.0f, 0.37f, 1.0f);
	...
	// wnd
	public		enum WINDOWID	{NONE=0, TOP_LEFT=10, TOP_CENTER, TOP_RIGHT, EFFECT_LIST, EFFECT_HIERARCHY, EFFECT_CONTROLS, TOOLIP_CURSOR, MODAL_MSG, MENUPOPUP, SPRITEPOPUP, POPUP=100, RESOURCE_START=200, HINTRECT=300};
...
//	protected	static Rect		GetWindowRect(float x, float y, float width, float height)		{ return new Rect(x, y, x+width, y+height); }
	...
	public		static Rect		GetToolMessageRect()				{ return new Rect(GetFixedWindowWidth()*2.5f, Screen.height-m_fActionToolbarHeight-m_rectOuterMargin.y-m_fToolMessageHeight-m_fTooltipHeight, Screen.width-GetFixedWindowWidth()*(m_nSidewindowWidthCount*2+1), m_fToolMessageHeight); }
	public		static Rect		GetTooltipRect()					{ return new Rect(m_rectOuterMargin.x*3+GetFixedWindowWidth()*m_nSidewindowWidthCount, Screen.height-m_fActionToolbarHeight-m_rectOuterMargin.y-m_fTooltipHeight, Screen.width-GetMenuChangeRect().width*4-m_rectOuterMargin.x*6, m_fTooltipHeight); }
